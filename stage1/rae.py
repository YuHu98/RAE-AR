...
class RAE(nn.Module):
    def __init__(self, 
        ...
        if pretrained_decoder_path is not None:
            ...
            # checkpoint = torch.load(pretrained_decoder_path, map_location='cpu')
            ...
            #         new_key = k.replace('decoder.', '') # 去掉前缀
            #         new_state_dict[new_key] = v
            #     else:
            #         new_state_dict[k] = v
            # keys = self.decoder.load_state_dict(new_state_dict, strict=False)
        ...
        if use_token_norm:
            ...
            print("Initialized Token-wise LayerNorm (Affine=False) for variance stabilization.")
        elif normalization_stat_path is not None:
    ...
    def encode(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = (x - self.encoder_mean.to(x.device)) / self.encoder_std.to(x.device)   # torch.Size([1, 3, 256, 256])
        z = self.encoder(x)   # torch.Size([1, 256, 768])
        ...
        return z   # torch.Size([1, 768, 16, 16])
