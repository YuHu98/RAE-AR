...
class VAE(nn.Module):
    ...
    def encode(self, x):   # torch.Size([32, 3, 256, 256])
        x = self.encoder(x)   # torch.Size([32, 256, 512])
        x = self.quant_proj(x)  # torch.Size([32, 256, 17])
        mu = x[..., :-1]
        kappa = x[..., -1]
        mu = l2_norm(mu)
        ...
        x = qz.rsample()    # torch.Size([32, 256, 16])
        x = x * (self.latent_dim**0.5)
        return x, loss.mean()
