...
from RAE_AR.dataset import build_dataset
from RAE_AR.gan.gan_loss import GANLoss
from RAE_AR.model import create_model, get_model_args
from RAE_AR.utils import create_logger, requires_grad, update_ema
...
def main(args):
    ...
    if args.vae_path != "":
        vae_info = torch.load(args.vae_path, map_location="cpu", weights_only=False)
        vae_info = vae_info["model"]
        res = model.load_state_dict(vae_info, strict=False)
        for k in res.missing_keys:
            if "vae" in k:
                raise ValueError(f"Fail to load VAE weights from {args.vae_path}")
        model.freeze_vae()
        logger.info(f"loaded pretrained VAE from {args.vae_path}")
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        if rank == 0 and epoch % 10==0:
            ...
            # local_file = os.path.join(args.tmp_results_dir, f"epoch_{epoch}.pt")
            local_file = os.path.join(args.results_dir, f"epoch_{epoch}.pt")
            torch.save(checkpoint, local_file)
            local_file = os.path.join(args.results_dir, f"last.pt")
            torch.save(checkpoint, local_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mixed-precision", type=str, default="bf16", choices=["none", "bf16"])
    ...
    parser.add_argument("--use-token-norm", action="store_true")
